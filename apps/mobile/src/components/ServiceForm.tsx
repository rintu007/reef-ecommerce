import { SERVICE_TYPE_LABELS, createService, updateService, type MarketType, type Service, type ServiceType } from "@reef-market/shared";
...
export function ServiceForm({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const service =
        mode === "create" ? (await createService(apiClient, payload)).service : (await updateService(apiClient, serviceId!, payload)).service;
      ...
      >
        {submitting ? (
          ...
          <Text className={`font-semibold text-sm ${canSubmit ? "text-white" : "text-muted-foreground"}`}>
            {uploadingCount > 0 ? "Uploading photos…" : mode === "create" ? "Post Service" : "Save Changes"}
          </Text>
        )}
      </Pressable>
    </ScrollView>
